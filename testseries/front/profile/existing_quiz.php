...
<script>
	...
	// }
	function add_quiz(){
		$('#right_sidebar').append('<br><input style = "height:30px;" type = "text" id = "quiz_name" placeholder = "Quiz Name">&nbsp;<select style = "height:30px;" id = "category_id">'+category+'</select>&nbsp;<input style = "height:30px;" type = "number" id = "duration" placeholder = "Duration">&nbsp;<button class = "button is-link" style="height:29px;padding-top:0px;font-size:12px;" onclick = "save_quiz()">SAVE</button>');
	}
	function save_quiz(){
		...
		$.ajax({
			url : '../../back/profile/existing_quiz.php',
			type : 'post',
			data : ({'chek':'new_quiz','quiz_name':quiz_name,'category_id':category_id,'duration':duration}),
			success:function(response){
				location.reload();
			}
		})
