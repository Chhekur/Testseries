...
<script>
	...
	function change_quiz(id,quiz){
		$('#quiz_change_user_id'+id).html('<input type = "text" id = "quiz" value = "'+quiz+'"><br><br><button style="height:24px;padding-top:0px;font-size:12px;" class = "button is-link" onclick = "save_quiz('+id+')">Change</button>');
	}
	function save_quiz(id){
		quiz = $('#quiz').val();
		$.ajax({
			url : '../../back/profile/active_users.php',
			type : 'post',
			data : ({'chek':'change_quiz','id':id,'quiz':quiz}),
