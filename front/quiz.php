...
include_once "../back/dashboard_left.php";
	...
	function submit(){
		...
		for(var i = 0; i < subjects.length; i++){
			...
			for (var j = 0; j < data[subjects[i]].length; j++){
				...
				if (correct[i][j] == data[subjects[i]][j]['answer'])
					ans++;
				if(correct[i][j] != 0 && correct[i][j] != data[subjects[i]][j]['answer']){
		...
		var score = ans - ((negative_marking != 0) ? wrong_ans/negative_marking : 0).toFixed(2);
		score --;
		$('#result').html('<center><h3>RESULT</h3></center><hr><div style = "margin-bottom:40px;"><span style = "float:left;">Your Rank : '+rank+'</span><span style = "float:right;">You Scored '+ score +' Out of '+no_of_question+'</span></div>');
