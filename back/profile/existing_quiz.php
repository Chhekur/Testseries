...
if($_POST['chek'] == 'get_quiz'){
	...
	//}//print_r($category);
	$i = 1;
	$response .= "<table width=100%><tr style = 'border-bottom:1px solid #dbdbdb;'><th style = 'padding-bottom:20px;'>Quiz Name</th><th style = 'padding-bottom:20px;'>Category</th><th style = 'padding-bottom:20px;'>Subjects</th><th style = 'padding-bottom:20px;'>Neg. Mark.</th><th style = 'padding-bottom:20px;'>Extra</th></tr>";
	while($row = mysqli_fetch_array($result)){
		...
		$subject .= '<table width=80%; id = "subject_table'.$row['id'].'">';
		...
		$subject .= '</table>';
		$response.= "<tr style = 'border-bottom:1px solid #dbdbdb;'><td style = 'padding-top:20px;padding-bottom:20px;'><h4>{$i}&nbsp;&nbsp;&nbsp;&nbsp;{$row['quiz_name']}</h4></td><td style = 'padding-top:20px;padding-bottom:20px;'><h4>{$ccategory}</h4></td><td style = 'padding-top:20px;padding-bottom:20px;'><h4>{$subject}</h4></td><td style = 'padding-top:20px;padding-bottom:20px;'><div id = 'edit_negative_marking{$row['id']}'><h4>{$row['negative_marking']}</h4><button class = 'button is-warning' style='height:20px;padding-top:0px;font-size:12px;' onclick = 'edit_negative_marking({$row['id']})'>Edit</button></div></td><td style = 'padding-top:20px;padding-bottom:20px;'><button class = 'button is-danger' style='height:20px;padding-top:0px;font-size:12px;' onclick = 'delete_quiz({$row['id']})'>Delete</button><br><button class = 'button is-warning' style='height:20px;padding-top:0px;font-size:12px;width:100px;' onclick = 'add_subject({$row['id']})'>Add Subject</button></td></tr>";
...
elseif($_POST['chek'] == 'new_quiz'){
	...
	mysqli_query($con,"INSERT into quiz (category_id,quiz_name,duration,negative_marking) values ('{$category_id}','{$quiz_name}','{$duration}','{$negative_marking}')");
	exit('success');
}
...
elseif($_POST['chek'] == 'save_negative_marking'){
	$negative_marking = $_POST['negative_marking'];
	$quiz_id = $_POST['quiz_id'];
	mysqli_query($con,"UPDATE quiz set negative_marking = '{$negative_marking}' where id = '{$quiz_id}'");
	exit('success');
}
